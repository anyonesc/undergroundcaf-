...
bot.on("guildMemberAdd", async member => {
    ...
    .setFooter(`Underground© Todos os direitos reservados.`, `${member.user.avatarURL}`)

    let canalEntrada = bot.channels.get("771207103866994738")
    .send(embed)
...
bot.on("guildMemberRemove", async member => {
    ...
    .setFooter(`Mensagem será deletada em 5s`, `${member.user.avatarURL}`)

    let canalEntrada = bot.channels.get("771207103866994738").send(embed)
});
...
bot.on('ready', () => {
    ...
    let status = [
        {name: 'Use /help caso tenha dúvida.', type: 'Ouvindo', url: 'Radio Underground'},
        {name: 'UNDERGROUND', type: 'WATCHING', url: 'https://www.twitch.tv/'},
